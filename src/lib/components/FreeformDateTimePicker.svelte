<script lang="ts">
    ...
    let { selectedDateTime = "", onDateTimeChange }: Props = $props();
    ...
    onMount(() => {
        ...
        fp = flatpickr(datepickerContainer as HTMLElement, {
            ...
            onChange: (selectedDates, dateStr) => {
                ...
                if (selectedTime) {
                    onDateTimeChange(`${dateStr}T${selectedTime}:00`);
                }
            },
        });
            ...
            >
                {#each timeSlots as slot}
                    <button
                        ...
                        class="group relative px-4 py-4 text-sm font-bold rounded-xl border-2 transition-all duration-300 {selectedTime ===
                        slot
