...
<div
    class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm text-start font-inter"
>
    <h3 class="text-xl font-bold text-gray-900 mb-6">
        {$t("patient_details.prescription_new_title")}
    </h3>
    ...
    <div class="relative mb-8">
        <label
            ...
            >{$t("patient_details.prescription_search_meds")}</label
        >
        <div class="flex gap-3">
            <div class="relative flex-grow">
                <input
                    ...
                    placeholder={$t(
                        "patient_details.prescription_search_placeholder",
                    )}
                />
                {#if showSuggestions && searchTerm}
                    <div
                        class="absolute z-20 mt-2 w-full bg-white shadow-2xl rounded-2xl py-2 text-base ring-1 ring-gray-100 overflow-hidden max-h-64 overflow-y-auto custom-scrollbar"
                    >
                        ...
                        {#if filteredMedications.length === 0}
                            ...
                            >
                                <span class="text-sm"
                                    >{$t(
                                        "patient_details.prescription_add_custom",
    ...
    {#if selectedItems.length > 0}
        <div class="space-y-6 mb-8">
            {#each selectedItems as item, i}
                <div
                    class="p-6 bg-gray-50/50 rounded-2xl border border-gray-100 relative group transition-all hover:bg-white hover:shadow-md"
                >
                    <button
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div class="md:col-span-1">
                            <h4 class="text-lg font-black text-indigo-600 mb-1">
                                {item.medication_name}
                            </h4>
                            <span
                                class="text-[10px] font-bold text-gray-400 uppercase tracking-widest"
                                >{$t(
                                    "patient_details.prescription_item_medication",
                                )}</span
                            >
                        </div>
                        <div>
                            <label
                                ...
                                >{$t(
                                    "patient_details.prescription_dosage",
                                )}</label
                            >
                            <input
                                type="text"
                                ...
                                placeholder={$t(
                                    "patient_details.prescription_dosage_placeholder",
                                )}
                            />
                        </div>
                        <div>
                            <label
                                class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5"
                                >{$t(
                                    "patient_details.prescription_duration",
                                )}</label
                            >
                            <input
                                type="text"
                                ...
                                class="block w-full border-gray-200 rounded-xl bg-white focus:ring-2 focus:ring-indigo-500 border p-2.5 text-sm font-medium"
                                placeholder={$t(
                                    "patient_details.prescription_duration_placeholder",
                                )}
                            />
                        </div>
                        <div class="md:col-span-3">
                            <label
                                class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5"
                                >{$t(
                                    "patient_details.prescription_instructions",
                                )}</label
                            >
                            <input
                                type="text"
                                ...
                                class="block w-full border-gray-200 rounded-xl bg-white focus:ring-2 focus:ring-indigo-500 border p-2.5 text-sm font-medium"
                                placeholder={$t(
                                    "patient_details.prescription_instructions_placeholder",
                                )}
                            />
                        </div>
                    </div>
        ...
        <div class="mb-8 grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label
                    ...
                    class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2"
                    >{$t("patient_details.prescription_type")}</label
                >
                <select
            ...
            <div>
                <label
                    ...
                    class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2"
                    >{$t("patient_details.prescription_notes_label")}</label
                >
                <textarea
                    ...
                    class="block w-full border-gray-200 rounded-xl bg-gray-50/50 focus:bg-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all font-medium py-3 px-4 border"
                    placeholder={$t(
                        "patient_details.prescription_notes_placeholder",
                    )}
                ></textarea>
            </div>
        </div>
    ...
    {:else}
        <div
            class="text-center py-16 bg-gray-50/50 rounded-3xl border-2 border-dashed border-gray-100 flex flex-col items-center gap-4"
        >
            <div
                class="w-16 h-16 bg-white rounded-full flex items-center justify-center shadow-sm text-indigo-200"
            >
                <svg
                    class="w-10 h-10"
                    fill="none"
            ...
            <p class="text-gray-400 font-bold">
                {$t("patient_details.prescription_empty_prompt")}
            </p>
        </div>
    {/if}
</div>
...
<style>
    ...
    :global([dir="rtl"]) .inset-inline-end-4 {
        left: 1rem;
    }
    :global([dir="ltr"]) .inset-inline-end-4 {
        right: 1rem;
    }
</style>
