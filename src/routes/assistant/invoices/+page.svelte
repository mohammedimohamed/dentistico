...
<div class="p-4 sm:p-6 lg:p-8">
    <div class="flex justify-between items-center mb-8">
        <div>
            <h2 class="text-2xl font-black text-gray-900">{$t('assistant.invoices.title')}</h2>
            <p class="text-gray-500 text-sm">{$t('assistant.invoices.description')}</p>
        </div>
        <button
            onclick={exportToCSV}
        ...
        >
            <span>📊</span> {$t('assistant.invoices.export.button')}
        </button>
    </div>
    ...
    <div class="bg-white shadow-sm border border-gray-100 rounded-2xl p-6 mb-8">
        <form class="grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
            <div class="md:col-span-2">
                <label for="search" class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">{$t('assistant.invoices.filters.searchLabel')}</label>
                <input
                    type="text"
                    name="search"
                    id="search"
                    bind:value={search}
                    placeholder={$t('assistant.invoices.filters.searchPlaceholder')}
                    class="w-full border-gray-100 bg-gray-50 p-3 rounded-xl focus:ring-indigo-500 focus:border-indigo-500 text-sm font-medium"
                >
            </div>
            <div>
                <label for="startDate" class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">{$t('assistant.invoices.filters.fromDate')}</label>
                <input
                    type="date"
                    name="startDate"
                    id="startDate"
            ...
            <div>
                <label for="endDate" class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">{$t('assistant.invoices.filters.toDate')}</label>
                <input
                    type="date"
                    name="endDate"
                    id="endDate"
            ...
            <div class="md:col-span-4 flex justify-end gap-3 pt-2">
                <a href="/assistant/invoices" class="text-xs font-bold text-gray-400 hover:text-gray-600 self-center mr-4">{$t('assistant.invoices.filters.reset')}</a>
    ...
    <div class="bg-white shadow-sm border border-gray-100 rounded-2xl overflow-hidden">
        <table class="min-w-full divide-y divide-gray-100">
            <thead class="bg-gray-50/50">
                <tr>
                    <th class="px-6 py-4 text-left text-[10px] font-bold text-gray-400 uppercase tracking-widest">{$t('assistant.invoices.table.headers.number')}</th>
                    <th class="px-6 py-4 text-left text-[10px] font-bold text-gray-400 uppercase tracking-widest">{$t('assistant.invoices.table.headers.patient')}</th>
                    <th class="px-6 py-4 text-left text-[10px] font-bold text-gray-400 uppercase tracking-widest">{$t('assistant.invoices.table.headers.date')}</th>
                    <th class="px-6 py-4 text-left text-[10px] font-bold text-gray-400 uppercase tracking-widest">{$t('assistant.invoices.table.headers.amount')}</th>
                    <th class="px-6 py-4 text-left text-[10px] font-bold text-gray-400 uppercase tracking-widest">{$t('assistant.invoices.table.headers.status')}</th>
                    <th class="px-6 py-4 text-right text-[10px] font-bold text-gray-400 uppercase tracking-widest">{$t('assistant.invoices.table.headers.actions')}</th>
                </tr>
            </thead>
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td class="px-6 py-5 whitespace-nowrap">
                            <span class="px-3 py-1 text-[10px] font-black uppercase tracking-widest rounded-full
                                ...
                                {$t(`assistant.invoices.table.status.${invoice.status}`)}
                            </span>
                        </td>
